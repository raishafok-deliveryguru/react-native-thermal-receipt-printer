...
export function exchange_text(text: string, options: IOptions): Buffer {
	...
	if (
		m_options['encoding'] &&
		options_controller['encoding'][m_options['encoding'] as keyof typeof encoding_mappings_bytes]
	) {
		bytes.concat(
			options_controller['encoding'][m_options['encoding'] as keyof typeof encoding_mappings_bytes]
		);
...
export async function exchange_image(imagePath: string, threshold: number): Promise<Buffer> {
	let bytes = new BufferHelper();
